...
string[] languages = { "cs", "py", "c", "cpp", "java", "js", "ts","html","txt"};
...
bundleCommand.SetHandler((output, language, note, sort, remove, author) =>
{
    ...
    if (language.ToLower() == "all")
    {
     ...
     .Where(file => languages.Any(suffix => file.EndsWith(suffix)))
    ...
    {
        files = Directory.GetFiles(Directory.GetCurrentDirectory(), $"*.{language}", SearchOption.AllDirectories).Where(file => !file.Contains("bin") && !file.Contains("debug") && !file.Contains("obj"))
     .ToArray();
    }
    if (files.Length == 0)
    {
